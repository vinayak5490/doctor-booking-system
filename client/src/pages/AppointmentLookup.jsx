...
export default function AppointmentLookup() {
    ...
    const handleSearch = (e) => {
      ...
      setLoading(false);
    }, 80000 / 100); // Quick 800ms mock loader
    }
  return (
        ...
        <div className='max-w-md w-full mx-auto space-y-6'>
            ...
            <div className='bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4'>
                <div className='text-center'>
                    <h2 className='text-2xl font-bold text-gray-900'>Find Your Appointment</h2>
                    <p className='text-sm text-gray-500 mt-1'>Enter your registered mobile number to check details</p>
                </div>
                    ...
                    {searchResult ? (
                        <div className='bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4'>
                            <div className='flex justify-between items-center border-b pb-3'>
                                <div>
                                    <span className='text-xs font-mono text-gray-400'>{searchResult.bookingId}</span>
                                    <h3 className='font-bold text-gray-950 text-lg'>{searchResult.doctorName}</h3>
                                </div>
                            ...
                            <div className='space-y-2 text-sm text-gray-600'>
                                <div className='flex justify-between'>
                                    <span>Patient:</span>
                                    <span className='font-medium text-gray-900'>{searchResult.patientName}</span>
                                </div>
                                <div className='flex justify-between'>
                                    <span>Date:</span>
                                    <span className='font-medium text-gray-900'>{searchResult.date}</span>
                                </div>

                                <div className='flex justify-between'>
                                    <span>Time Slot:</span>
                                    <span className='font-medium text-blue-600'>{searchResult.time}</span>
                                </div>
                            </div>
                            ...
                            <div className='grid grid-cols-2 gap-3 pt-2 border-t'>
                                <button
                                onClick={() => navigate("/reshedule-appointment", {state : {appointment : searchResult}})}
                                className='py-2 text-sm font-medium border border-gray-200 text-gray-700 rounded-xl hover:bg-gray-50 transition'
                                >
                                    Reschedule
                                </button>
                                <button
                                onClick={() => navigate("/cancel-appointment", {state: {appointment: searchResult}})}
